...
export const GSAPRevolvingServices = () => {
  ...
  useGSAP(
    ...
    { scope: containerRef, dependencies: [stepAngle, totalItems, viewMode] }
  );
  ...
  return (
    <section
      ref={containerRef}
      id="services"
      className={`relative w-full bg-white text-zinc-900 overflow-hidden flex flex-col justify-between py-6 sm:py-8 md:py-10 lg:py-12 px-4 sm:px-6 lg:px-12 selection:bg-zinc-900 selection:text-white border-t border-zinc-200 ${viewMode !== "desktop" ? "min-h-auto" : "min-h-screen"}`}
    >
      {/* Header Controls */}
        ...
        {viewMode === "mobile" && (
          <div className="flex items-center gap-2 pt-2">
            <button
              ...
              className="p-2.5 rounded-full border border-zinc-300 bg-white text-zinc-800 shadow-xs hover:bg-zinc-100 flex items-center justify-center min-h-[44px] min-w-[44px]"
            ...
            <span className="text-[11px] font-mono text-zinc-500 ml-2">Swipe or tap arrows</span>
          </div>
        )}
      </header>

      {viewMode === "mobile" ? (
        // Mobile: Snap-scrolling horizontal cards with clean styling
        <main className="max-w-7xl w-full mx-auto my-auto pt-6 pb-4 z-10">
          <div
            ref={scrollContainerRef}
            className="flex gap-4 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-hide px-1"
          >
            {SERVICES.map((service, idx) => {
              ...
              return (
                <article
                  key={service.id}
                  className="flex-shrink-0 min-w-[280px] max-w-[85vw] rounded-2xl select-none flex flex-col overflow-hidden border border-zinc-200 shadow-md snap-center bg-white"
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <span
                      className="absolute top-3 left-3 text-xs font-black font-mono tracking-wider px-2 py-0.5 rounded-md bg-white/90 shadow-sm"
                  ...
                  <div className="p-4 flex flex-col justify-between flex-1">
                    <div>
                      <h3 className="text-lg font-black uppercase tracking-tight text-zinc-950 leading-tight">
                        {service.title}
                      </h3>
        ...
        // Tablet (768-1023px): 2-Column Static Grid of Service Cards
        <main className="max-w-7xl w-full mx-auto my-auto pt-8 pb-8 z-10">
          <div className="grid grid-cols-2 gap-6">
            {SERVICES.map((service, idx) => {
              ...
              return (
                <article
                  key={service.id}
                  className="rounded-2xl flex flex-col overflow-hidden border border-zinc-200 shadow-sm hover:shadow-md transition-shadow bg-white"
                >
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/80 via-zinc-900/30 to-transparent" />
                    <span
                      className="absolute top-3 left-3 text-xs font-black font-mono tracking-wider px-2.5 py-1 rounded-md bg-white/90 shadow-sm"
                      style={{ color: acc.accent }}
                    >
                      {service.number}
                    </span>
