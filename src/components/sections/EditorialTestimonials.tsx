...
export const EditorialTestimonials = () => {
  ...
  return (
    <section
      ...
      className="py-16 sm:py-20 lg:py-24 px-6 sm:px-12 bg-zinc-100 text-zinc-900 relative border-t border-zinc-200 select-none font-outfit"
    >
      <div className="max-w-[1700px] mx-auto space-y-16">
        {/* Header Bar */}
        <div className="gsap-testimonial-header flex flex-col sm:flex-row sm:items-end justify-between gap-6 border-b border-zinc-300 pb-8">
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div className="flex items-center gap-1 text-amber-500 bg-zinc-200/60 px-4 py-2 rounded-full border border-zinc-300">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-4 h-4 fill-current" />
              ))}
              <span className="text-xs font-outfit font-extrabold text-zinc-900 ml-2">5.0 RATING</span>
            </div>

            {/* Play/Pause Auto-Timer Toggle Button */}
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="text-xs font-outfit font-extrabold px-4 py-2 rounded-full border border-zinc-400 hover:bg-zinc-900 hover:text-white transition-all duration-300 flex items-center gap-2"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-8 space-y-8">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="space-y-6"
              >
                <Quote className="w-12 h-12 text-zinc-600" />

                <blockquote className="text-2xl sm:text-4xl font-outfit font-light leading-snug tracking-tight text-zinc-900 italic">
                  "{current.quote}"
                </blockquote>

                <div className="flex items-center gap-4 pt-4 border-t border-zinc-300">
                  <img
                    src={current.avatar}
                    alt={current.author}
                    className="w-14 h-14 rounded-full object-cover border-2 border-zinc-300 shadow-md"
                  />
                  <div>
                    <span className="font-outfit font-black text-lg uppercase text-zinc-900 block">
                      {current.author}
                    </span>
          ...
          {/* Metric Box & Controls */}
          <div className="gsap-testimonial-card lg:col-span-4 p-8 sm:p-10 rounded-3xl bg-white border border-zinc-300 space-y-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              <div className="text-3xl sm:text-4xl font-outfit font-black text-zinc-900 uppercase">
                {current.metric}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-zinc-300 pt-6">
              ...
              <div className="flex items-center gap-3">
                <Magnetic strength={0.3}>
                  <button
                    onClick={handlePrev}
                    className="p-3.5 rounded-full border border-zinc-400 hover:bg-zinc-900 hover:text-white transition-colors"
